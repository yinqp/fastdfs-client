...
package org.ifdow.fastdfs;

import org.ifdow.common.IniFileReader;
import org.ifdow.common.MyException;
...
public class ClientGlobal {
	...
	public static class Builder {
		...
		public Builder setTrackerServers(String trackerServers) {
		...
		public Builder setNetworkTimeout(String networkTimeout) {
		...
		public Builder setAntiStealToken(String antiStealToken) {
		...
		public Builder setTrackerHttpPort(String trackerHttpPort) {
			this.trackerHttpPort = trackerHttpPort;
			return this;
		}

		public Builder setConnectionPoolEnabled(String connectionPoolEnabled) {
			this.connectionPoolEnabled = connectionPoolEnabled;
			return this;
		}

		public Builder setConnectionPoolMaxCountPerEntry(String connectionPoolMaxCountPerEntry) {
			this.connectionPoolMaxCountPerEntry = connectionPoolMaxCountPerEntry;
			return this;
		}

		public Builder setConnectionPoolMaxIdleTime(String connectionPoolMaxIdleTime) {
			this.connectionPoolMaxIdleTime = connectionPoolMaxIdleTime;
			return this;
		}

		public Builder setConnectionPoolMaxWaitTimeInMs(String connectionPoolMaxWaitTimeInMs) {
		...
		public Properties build() {
			...
			p.setProperty(PROP_KEY_TRACKER_SERVERS, this.trackerServers);
			if (this.connectTimeout != null && this.connectTimeout.trim().length() > 0)
				p.setProperty(PROP_KEY_CONNECT_TIMEOUT_IN_SECONDS, this.connectTimeout);
			
			if (this.networkTimeout != null && this.networkTimeout.trim().length() > 0)
				p.setProperty(PROP_KEY_NETWORK_TIMEOUT_IN_SECONDS, this.networkTimeout);
			
			if (this.charset != null && this.charset.trim().length() > 0)
				p.setProperty(PROP_KEY_CHARSET, this.charset);
			
			if (this.antiStealToken != null && this.antiStealToken.trim().length() > 0)
				p.setProperty(PROP_KEY_HTTP_ANTI_STEAL_TOKEN, this.antiStealToken);
			
			if (this.secretKey != null && this.secretKey.trim().length() > 0)
				p.setProperty(PROP_KEY_HTTP_SECRET_KEY, this.secretKey);
			
			if (this.trackerHttpPort != null && this.trackerHttpPort.trim().length() > 0)
				p.setProperty(PROP_KEY_HTTP_TRACKER_HTTP_PORT, this.trackerHttpPort);
			
			if (this.connectionPoolEnabled != null && this.connectionPoolEnabled.trim().length() > 0)
				p.setProperty(PROP_KEY_CONNECTION_POOL_ENABLED, this.connectionPoolEnabled);
			
			if (this.connectionPoolMaxCountPerEntry != null && this.connectionPoolMaxCountPerEntry.trim().length() > 0)
				p.setProperty(PROP_KEY_CONNECTION_POOL_MAX_COUNT_PER_ENTRY, this.connectionPoolMaxCountPerEntry);
			
			if (this.connectionPoolMaxIdleTime != null && this.connectionPoolMaxIdleTime.trim().length() > 0)
				p.setProperty(PROP_KEY_CONNECTION_POOL_MAX_IDLE_TIME, this.connectionPoolMaxIdleTime);
			
			if (this.connectionPoolMaxWaitTimeInMs != null && this.connectionPoolMaxWaitTimeInMs.trim().length() > 0)
				p.setProperty(PROP_KEY_CONNECTION_POOL_MAX_WAIT_TIME_IN_MS, this.connectionPoolMaxWaitTimeInMs);
